...
class QNScriptMessage: NSObject, WKScriptMessageHandler {
...
extension WKUserContentController {
    func addHandler(_ message: Any, name: String) {
        if let msg = message as? WKScriptMessageHandler {
            self.add(QNScriptMessage(msg), name: name)
        }
    }
}
...
class WebView: UIView {
    ...
    // 通过监听获取当前URL
    private(set) var currentRequestURL : URL?
    ...
    fileprivate func setupUI(webConfig:WKWebViewConfig)  {
        ...
        configuretion.allowsInlineMediaPlayback = true // 网页视频内嵌播放
        _ = webConfig.scriptMessageHandlerArray.map{
            [weak self] in
            if let weakSelf = self {
                configuretion.userContentController.addHandler(weakSelf, name: $0)
            }
        }
        
        _ = webConfig.userScriptArray.map{configuretion.userContentController.addUserScript($0)}
    ...
    // 增加显示区域
    public func changeInsetTop(_ top: CGFloat) {
        webView.scrollView.insetTop = top
    }
    ...
    public func pauseVideo(){
        webView.evaluateJavaScript("var videos = document.getElementsByTagName('video');for (var i = 0; i < videos.length; i++) {videos[i].pause();}", completionHandler: nil)
    }
    
    /// 暂停音频
    public func pauseAudio() {
        if (webView.title?.contains("音乐"))!{
           webView.reload()
        }
    }
    /// 移除webView
    ...
    open override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if keyPath == "estimatedProgress"{
            ...
            if let newValue = change?[NSKeyValueChangeKey.newKey] {
                currentRequestURL = newValue as? URL
            }
        }
    }
}
                ...
                if let strEncoding = urlString.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) {
                    ...
                    if let urlalipayURL:URL = URL(string: payString) {
                        if #available(iOS 10.0, *) {
                            UIApplication.shared.open(urlalipayURL, options: [:], completionHandler: {
                                [weak self] result in
                                self?.webView.reload()
                            })
                        } else {
